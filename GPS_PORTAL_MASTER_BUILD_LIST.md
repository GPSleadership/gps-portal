...
| # | Item | Effort | Status | Evidence |
|---|------|--------|--------|----------|
| **P1-MSG1** | **Messages allow only ONE attachment at a time.** Alex hit this sending Sergio the PDF + interview questions together. Not a bug — by design: `coach.html:4638` file input has no `multiple`, `msgPickFile` reads only `files[0]`, and each message row stores a single attachment (`attachment_name`/`_size`/`_download`). Raise to **3–5 per message**: add `multiple` to the file inputs (coach + client composers), a `message_attachments` child table (one→many), multi-file upload handling in the send path, and render multiple file cards per message. Client + coach composers both. | M | ⏳ **queued — soon** | coach.html:4638/4670/4685; client.html composer; message schema |
...
| **P1-M5b** | Phase 5 part 2 — **AI-drafted talking track** on the Coaching Moment (grounded in the brief + diagnostic + transcripts, reusing the library-prompt system; coach reviews, never auto-sends). Respects owner/assistant roles. | M | ⏳ queued | — |
