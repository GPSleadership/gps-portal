...
**P0 — `self_checks` checkpoint constraint rejected `day30`.** ✅ FIXED live 2026-07-22 (v117). Found via Sergio Sabido: he emailed "Error saving… not allowing me to proceed" with a screenshot of the "Quick self-assessment before you see your results" modal showing **"Error saving: Could not save self-check."** Root cause: the results-gate self-check sends `checkpoint='day30'` (client.html:7252; survey checkpoints are baseline/day30/day90), but the `self_checks_checkpoint_check` CHECK constraint only permitted `('day45','day90')` — a stale value from before the re-score cadence moved to 30 days. Every day30 self-check insert failed with 23514; `self_checks` was **empty** (silently broken for ALL leaders since the rename, not just Sergio). Reproduced the 23514, applied **v117** widening the constraint to `('baseline','day30','day45','day90')`, re-verified the insert succeeds, removed the test row. **Pure DB fix — no code deploy needed;** the app already sent the correct value. Migration file `supabase-migration-v117-selfcheck-checkpoint.sql` committed for the record.

### P1 — Make self-check checkpoint validation state-driven, not a frozen allowlist (added 2026-07-22, Alex, Friday window)
v117 unblocked everyone, but it's still a **hardcoded allowlist** kept in two places (DB CHECK constraint + the app's checkpoint labels). They drifted once (day45 vs day30) and broke every save; a future cadence change (back a leader off to a gentler rhythm, add a day60, etc.) could reintroduce the same class of bug until someone remembers to update the list. **Fix: tie the save to where the leader actually is.** In `portal-data.js` `add-self-check`, validate the submitted checkpoint against the stages the leader genuinely has survey responses for (their real state), or a single canonical checkpoint constant shared app-wide — reject unknowns with a clear client message, never a raw 500. Then the DB constraint can be a generous superset (or dropped in favor of the server gate). Net: "wherever they are" is always accepted by definition, and changing a cadence is a one-place edit, not a landmine. Also audit other checkpoint literals for day30/day45 drift (survey.js re-score labels, any "45-day re-score" copy). — S–M
...
### P1 — One-click follow-up email from the capture panel — BUILT 2026-07-22, on `feature/debrief-script`
Built mid-week (out of normal cadence) at Alex's explicit request — he has live debriefs Jana (Wed), Michael (Fri), Rosa (Mon) and wanted to dogfas it. **Draft → review/edit → send**, right in the debrief modal's capture panel.

- **`api/debrief-email.js`** (NEW, coach-gated — kept OUT of the extractive `debrief-script.js` so that endpoint stays zero-LLM). `draft` LLM-writes the follow-up in Alex's voice from THIS leader's captured data (focus, behavior/metric, value_end + appetite ratings, outcome, coach notes) using the 8-part `gps-debrief-followup` structure; `send` emails the coach-reviewed subject+body to the leader via Resend (from "Alex Tremble - GPS Leadership", reply-to the coach), logs the send on `debrief_captures`. `dry_run:true` builds the payload without sending.
- **Guardrails:** AI kill-switch `debrief_followup` (fail-open, template fallback if off/unavailable — never a hard error). Aggregates only, no rater quotes / other leaders / delicate perception findings (enforced in prompt). No dollar figures in the email. Bracketed [placeholders] for anything not captured, so Alex fills them on review. Requires a valid leader email; **coach clicks send — never auto-sends;** confirm dialog + placeholder warning.
- **`supabase-migration-v116-debrief-followup.sql`** — adds `followup_sent_at/to/subject` to `debrief_captures` (send-log) + registers the `debrief_followup` AI flag. Applied to prod 2026-07-22.
- **coach.html** — Follow-up email panel under the capture panel: Draft button → editable To/Subject/Body → Send. JS sweep + color-guard green.
- **Draft quality depends on saving the capture first** (ratings/notes populate the email). Focus auto-fill is thin because `results_narrative.focus1` is empty on current leaders — drafts bracket it for Alex. Pairs with the Fireflies API pull + the editable-script work below.
- **Status:** ⏳ built on branch, v116 applied to prod, pending push + live draft/send test.
...
- **Verified 2026-07-24: already satisfied in production, no change needed.** Rosa's real sponsor_teams link (`adf36d43-...`) is `confidentiality_mode='private'` (not `'summary'` — corrected from earlier notes). `api/sponsor-data.js` only populates `sprint_cta_url` when `!isPrivate && !isProgress`; in private mode it stays `null`, and `renderSprintCta()`/the sticky sprint bar/the approvals "start here" link in `decision-room.html` all no-op on a null URL. Confirmed by loading Rosa's actual token link directly (bypassing the coach-preview override, which forces standard view and would have hidden this) — no sprint CTA, no sticky bar, no price, anywhere on the page. Alex confirmed (2026-07-24): keep her link on `private` as-is — no code or data change.
- **Mechanism already exists for future engagements too:** `sponsor_teams.custom_cta_url = 'disabled'` forces the sprint CTA off even outside private/progress mode, if a coach ever wants full detail shown but no CTA. No coach.html UI for this yet — DB-only today.
- **Status:** ✅ verified, no build needed.

### P1-RATER1 — Rater-group filter toggle recomputes TP3 (and 9-box if feasible)
Let a viewer include/exclude rater groups and see TP3 scores (and 9-box, if feasible) recompute live — the "remove Other colleagues and the score aligns with the team" reveal. **Hard guardrails:** server-computed presets only (never client-side raw data), each shown group must have **≥3 raters** (min-N=3), never expose a sub-3 group alone. Supervisor n=1 is acceptable ONLY for Rosa/JMAA because Rosa IS the supervisor rating her own leaders — do not generalize this exception to other engagements. Default view = all raters; framing always points to "people closest to your work," never "remove the critics."
- **Confidentiality question resolved 2026-07-24:** Rosa's link stays `confidentiality_mode='private'` (see P1-UPSELL2 above) — so the rater-group decomposition does **not** reach her standing Decision Room view. Built **coach-side only** (inside coach.html's diagnostic Report tab), coach-controlled, not exposed on decision-room.html/sponsor-data.js.
- **Built 2026-07-24** (branch `rater-group-filter-v1`, JS sweep + color-guard clean): new "Rater-group view (coach only)" card in `renderReportTab()`, gated on `d.report_generated_at`. Reads the already-computed `by_group` blob off the latest `diagnostic_report_drafts` row (same min-N-safe data that already feeds leader/coach/sponsor views — never buckets raw `diagnostic_responses` itself, so it can't bypass the existing confidentiality wall). Checkboxes per qualifying group (only groups that already survived MIN_N=3, or the supervisor, ever appear); toggling recomputes an n-weighted blend of Trust/Proactivity/Productivity/TP3 client-side — arithmetic on already-aggregated numbers only, never raw scores. Defaults to all groups checked; shows an amber note when filtered down so it reads as a debrief-prep lens, not a publishable number. Self shown for reference only, never toggleable.
- **9-box explicitly out of scope at this grain:** the Talent & Readiness Grid is a team-level comparison across multiple leaders (decision-room.html), not something that recomputes within one leader's own rater-group breakdown — nothing to build here.
- **Status:** ✅ built, pending Alex branch/push/preview/test/merge.
...
- **Status:** ⏳ queued. **⚑ ALEX TO-DO:** record the 3–5 min EIS overview video — the build is blocked until the file exists. (Added to Alex's to-do 2026-07-24; he'll do it soon.)
...
| **P1-UX11** | **Polish the individual client profile** (Alex 2026-07-18, "doesn't look too nice/great, especially each profile"). Live look at /coach → Clients → a profile found: (1) **emoji icons in the left sidebar nav** (📊🔬🎯✅👥⚙️) read as amateur and clash with the brand — replace with clean simple icons or plain text labels (highest-impact fix, affects every profile tab); (2) **duplicated identity block** — name/avatar/org appear in the navy header AND again in the Summary card just below; de-dupe; (3) **flat dead-grey metric progress bar** even at 0% — use `--ok` green as it fills. All mobile-verified, all via tokens. Small-to-medium; the emoji swap alone is a quick high-impact win. | M | ✅ desktop polish shipped 2026-07-18 (SVG icon nav replacing emoji + de-duped identity strip + softened metric bar, verified on prod). **Mobile follow-up** (Alex, on his phone: the sidebar eats half the screen) — on ≤680px the section nav collapses to a full-width **dropdown** at the top and the content gets the whole page (native `<select>` synced to `switchProfileNav`, sidebar hidden). Branch `coach-profile-mobile-dropdown`; JS sweep + color-guard clean. **Follow-up fix 2026-07-18**: the dropdown shipped with a layout bug (empty content on mobile) — `overflow:visible` on `.profile-body` made it grow past the screen and clip the content. Fixed by keeping the body height-constrained + `min-height:0` on `.profile-content` so it scrolls internally; verified via live DOM simulation (body fits overlay, content scrolls). Branch `coach-profile-mobile-fix`. | live: /coach#clients profile |
| **P1-REV1** | **"Send renewal / re-up link" from each client profile** (Alex 2026-07-18, on mobile — revenue). In the coaching profile (Admin & Links, and ideally a visible button), let the coach pick a term (1 mo / 3 mo / custom) and email the client a pay-to-continue link so they can renew/pay without the coach leaving the dashboard. Reuse the existing payment-link + renewal infra (`organizations.payment_link`, decision-room "Pay online" / renewal links, the notify/send-email path). Build: an endpoint (`coach-data` action `send-renewal-link`) + a small UI in the profile + choose which link/term. Works on mobile. | M | ⏳ queued (P1 — revenue) | payment_link infra exists; profile Admin section |
| **P1-UX12** | **Make the client's diagnostic *results* one tap from the profile** (Alex 2026-07-18). Today the profile's Diagnostics & Reports card has "Open →" which works (`closeClientProfile();showSection('diagnostics');openDiag(id)`) but is unobvious and jumps out of the profile. Relabel to "View results →" and land directly on the report/results view (not just the diagnostic overview). Prominence over "+ Start Diagnostic" when a diagnostic already exists. | S | ⏳ queued | coach.html profile diagnostics section (~line 3060) |
...
**Preview test (2026-07-19):** eligible coaching TEST client → block renders on Review step, opt-in saves `sms_opt_in`+`phone`. Trial/workshop TEST client → `smsEligible=false`, block absent, no SMS fields written. **DISCOVERY:** Twilio env vars are ALREADY live on the Vercel project (`smsConfigured()` was true on preview — the confirmation actually fired to the test number). So the only remaining gate was campaign approval, and a review-window opt-in would have failed at the carrier yet still burned the dedupe stamp. **Fix:** added an `SMS_LIVE` master switch to `twilio-sms.js smsConfigured()` — every send path (opt-in confirmation, weekly reminders, ad-hoc coach texts) no-ops until `SMS_LIVE=true` is set in Vercel, which Alex flips ONLY after the A2P campaign is approved. Verified: creds-set + SMS_LIVE unset → false; SMS_LIVE=true → true. TEST rows deleted.

### P1 — SMS friction-killers (added 2026-07-19, Alex — right after the wizard opt-in)
| ID | Item | Effort | Status | Evidence |
|---|---|---|---|---|
| **P1-SMS2** | **Preload the cell number + send the portal link on opt-in.** (A) Added a **Mobile phone** field to the coach Add Client modal (`coach.html`, `#newPhone` → `addClient()` insert payload). When the coach captures the cell up front, the wizard's text-reminder step arrives **pre-filled** (client.html already prefills `wizPlan.phone` from `client.phone`) — so the leader just checks "Yes." (B) The one-time opt-in confirmation SMS (`portal-data.js update-client`) now also carries the leader's **private portal link** (`${PORTAL_BASE}/client?token=…`, new `PORTAL_BASE` const) so they can save it to their phone's home screen. Still gated by `SMS_LIVE` + campaign approval (no send until live). node --check + JS sweep + color-guard clean. **Post-deploy:** create a client with a number → open their wizard → confirm the opt-in number is pre-filled; after `SMS_LIVE=true`, confirm the confirmation text includes the working portal link. | S | ⏳ Built 2026-07-19 — branch, awaiting preview + merge | coach.html (Add Client modal + addClient), api/portal-data.js (PORTAL_BASE + link in opt-in SMS) |
