// GPS Leadership — Manual Reminder Trigger
// Called by the coach portal's "Run Reminders Now" button.
// Authenticates via coach password, then runs the full send-reminders logic.
...
export default async function handler(req, res) {
  ...
  const settingsRes = await fetch(
    `${SUPABASE_URL}/rest/v1/coach_settings?key=eq.coach_password&select=value`,
    { headers: { apikey: SUPABASE_ANON, Authorization: `Bearer ${SUPABASE_ANON}` } }
  );
  const settingsRows = await settingsRes.json();
  const storedPassword = settingsRows?.[0]?.value || 'GPS2026';

  // Also check admin accounts
  const adminRes = await fetch(
    `${SUPABASE_URL}/rest/v1/admin_accounts?is_active=eq.true&select=password`,
    { headers: { apikey: SUPABASE_ANON, Authorization: `Bearer ${SUPABASE_ANON}` } }
  );
  const adminAccounts = await adminRes.json();
  const adminPasswords = Array.isArray(adminAccounts) ? adminAccounts.map(a => a.password) : [];

  const isValid = password === storedPassword || adminPasswords.includes(password);
  if (!isValid) {
    return res.status(401).json({ error: 'Invalid password' });
  }

  // ── Fetch all active clients with a plan and email ────────────────────────────
  ...
  if (!Array.isArray(clients) || clients.length === 0) {
    return res.status(200).json({
      message: 'No active clients with email addresses found. Make sure clients have emails saved in their profiles.',
      sent: 0,
      skipped: [],
      errors: [],
    });
  ...
  if (toRemind.length === 0) {
    return res.status(200).json({
      message: 'All eligible clients have already submitted their check-in this week.',
      sent: 0,
      skipped,
      errors: [],
    });
